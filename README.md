...
| Zenn   | [T_Sumida](https://zenn.dev/t_sumida)    |
...
## 職務要約
機械学習・LLMを活用したプロダクト開発において、PoC、モデル検証、API/バッチ基盤の設計実装、ログ収集、監視、継続改善まで一貫して担当してきました。
特に、業務要件や運用制約を踏まえて、AI機能を本番導入可能なシステムへ落とし込むことを強みとしています。今後は Backend × AI 実装の強みを軸に、MLOps・データ基盤・継続改善の仕組みづくりを強化していきたいと考えています。

## 強み
- ML/LLMシステムのPoCから本番運用まで一気通貫で担当可能
- GCP/Azure/AWSを用いたAPI、バッチ、ログ収集、監視設計の実務経験
- 顧客折衝、要件定義、評価設計、改善提案まで含めて推進可能
- 小規模チームにおける技術選定・実装リード・ドキュメント整備に強み

## 志向
今後は、MLOps・データ基盤・継続改善の仕組みづくりを強化しつつ、機械学習を事業成果に接続するプロダクト開発に貢献したいと考えています。


## 技術スタック
- 言語：Python（主）、Rust（学習・一部開発）、C/C++
- ML/AI：PyTorch, TensorFlow, 画像認識, 信号処理, LLM, RAG
- Backend：FastAPI, Flask, Docker, バッチ処理, Web API
- Cloud：GCP, AWS, Azure
- Data/MLOps：BigQuery, Terraform, MLflow, ログ収集, 評価, 監視
- その他：Kubernetes, GitHub Actions, React（簡易モックレベル）

## 補足
主にバックエンド/ML/データ基盤領域を担当してきました。フロントエンドは簡易モックや検証用途での実装が中心です。


## 強みのある領域
### LLM / 生成AI
- Azure OpenAI Service, Google Cloud Platform を用いた対話システム・推薦システム・大規模バッチ処理基盤の設計/実装
- RAGを用いた検索・応答アーキテクチャ設計、評価、改善
- プロンプト改善、品質評価、監視設計、運用フロー構築

### MLプロダクト開発
- 画像認識・信号処理案件におけるPoC、モデリング、API化、本番運用
- 業務要件に応じた評価指標設計、レポーティング、自動化

### Backend / Cloud / MLOps
- FastAPI/Flask を用いたML API・バッチシステム開発
- GCP/AWS/Azure上でのデプロイ、ログ設計、監視、運用改善
- Terraform、BigQuery、GitHub Actions 等を用いた開発/運用基盤整備
...
#### LLMを用いた大規模バッチ処理システムの開発

- プロジェクト概要
  - ユーザー属性と業務情報をもとに、各ユーザー向けの推薦コメントをLLMで大量生成するバッチシステムを開発。毎朝09:00〜12:00の間に全件処理を完了する必要があり、3万〜10万件/日のデータを約2時間で処理できる構成をGCP上に設計・実装した。

- 担当領域
  - PM：1名、バックエンド：1名、バックエンド＋AI：1名
  - 自身は、バッチ基盤およびLLM連携部分の設計・実装を担当。

- 取り組み
  - Cloud Tasks、Cloud Run Functions、Cloud Scheduler を組み合わせ、LLM APIごとの TPM/RPM 制約に応じて処理速度を制御できる並列バッチ基盤を設計・実装。
  - APIごとにキューを分離して負荷分散を行い、時間制約のあるワークフローでも安定して処理できる構成を構築。
  - 指定時間までに全件処理が完了しない場合でも、途中結果を安全に次工程へ引き渡せるフォールバック処理を実装。
  - キャッシュが効きやすいプロンプト設計を導入し、当初想定比で約50%のAPIコスト削減を実現。
  - Cloud Monitoring と構造化ログを用いて、API利用量・処理速度・エラー状況を可視化するダッシュボードを整備し、異常時の Slack / メール通知を含む監視体制を構築。
  - リトライ有無や失敗時の扱いを顧客運用に応じて切り替えられるようにし、実運用に合わせた柔軟な制御を可能にした。

- 工夫した点・成果
  - 当初想定されていた Cloud Run Jobs ではなく、実装期間・制御性・拡張性を踏まえて Cloud Run Functions + Cloud Tasks 構成へ切り替え、短期間での構築と運用しやすさを両立。
  - 3万〜10万件/日の推薦コメント生成を約2時間で安定処理できるバッチ基盤を実現。
  - APIコストを当初想定比で約50%削減。
  - 本番運用開始後の障害発生は月1件程度に抑え、監視・通知により早期検知可能な体制を整備。た。
